-- TÜM RLS HATALARINI TEK SEFERDE DÜZELT
-- Bu script Supabase SQL Editor'de çalıştırılmalı

-- 1. ÖNCE TÜM MEVCUT POLİTİKALARI TEMİZLE
DO $$
DECLARE
    r RECORD;
BEGIN
    ...
    END LOOP;
    RAISE NOTICE 'Tüm mevcut politikalar temizlendi';
END$$;
...
-- 12. PROJECT_TASK_STATUSES - Proje sahibi ve üyeleri görebilir, sadece sahip düzenleyebilir
...
-- 13. TASKS - Proje sahibi ve üyeleri görebilir, sadece sahip düzenleyebilir
...
-- 14. TASK_COMMENTS - Proje sahibi ve üyeleri görebilir, sadece sahip düzenleyebilir
...
-- 15. TASK_TIME_LOGS - Proje sahibi ve üyeleri görebilir, sadece sahip düzenleyebilir
...
-- 16. TASK_ACTIVITIES - Proje sahibi ve üyeleri görebilir
...
-- 17. TASK_ASSIGNMENTS - Proje sahibi ve üyeleri görebilir, sadece sahip düzenleyebilir
...
-- 18. TASK_DEPENDENCIES - Proje sahibi ve üyeleri görebilir, sadece sahip düzenleyebilir
CREATE POLICY "task_dependencies_select_team" ON public.task_dependencies FOR SELECT USING (
    EXISTS (SELECT 1 FROM public.project_tasks t 
            JOIN public.projects p ON t.project_id = p.id 
            WHERE t.id = task_dependencies.task_id AND (p.owner_id = auth.uid() OR 
            EXISTS (SELECT 1 FROM public.project_members WHERE project_id = p.id AND user_id = auth.uid())))
);
CREATE POLICY "task_dependencies_insert_owner" ON public.task_dependencies FOR INSERT WITH CHECK (
    EXISTS (SELECT 1 FROM public.project_tasks t 
            JOIN public.projects p ON t.project_id = p.id 
            WHERE t.id = task_dependencies.task_id AND p.owner_id = auth.uid())
);
CREATE POLICY "task_dependencies_update_owner" ON public.task_dependencies FOR UPDATE USING (
    EXISTS (SELECT 1 FROM public.project_tasks t 
            JOIN public.projects p ON t.project_id = p.id 
            WHERE t.id = task_dependencies.task_id AND p.owner_id = auth.uid())
);
CREATE POLICY "task_dependencies_delete_owner" ON public.task_dependencies FOR DELETE USING (
    EXISTS (SELECT 1 FROM public.project_tasks t 
            JOIN public.projects p ON t.project_id = p.id 
            WHERE t.id = task_dependencies.task_id AND p.owner_id = auth.uid())
);

-- 19. TASK_FILES - Proje sahibi ve üyeleri görebilir, sadece sahip düzenleyebilir
...
-- 20. TASK_TAGS - Herkes görebilir, sadece giriş yapmış kullanıcılar düzenleyebilir
...
-- 21. TASK_TAG_RELATIONS - Proje sahibi ve üyeleri görebilir, sadece sahip düzenleyebilir
...
-- 22. NOTIFICATIONS - Kullanıcı sadece kendi bildirimlerini görebilir
...
-- 23. USER_PREFERENCES - Kullanıcı sadece kendi tercihlerini görebilir
